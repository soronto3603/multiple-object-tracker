...
def dump(file, dest_path ,dump_embedded=False, dump_raw_blocks=False, extended_scan=False, verbose=False):
                    ...
                    if 'data' in content and ctype == 'free':
                        ...
                        while idx < len(data):
                            ...
                            if block_data[0:4] in emb_file_def:
                                ...
                                if ext is not None:
                                    print(dest_path + "output" + ext)
                                    with open(dest_path + "output" + ext, "wb") as ofd:
                                        ofd.write(block_data[first:last])

                                if ext == '.3gf':
                                    with open(dest_path + "output" + ext+".txt", "wt") as ofd:
                                        n=0
                                        while first < last:
